...
type DraggableSliderProps = {
  ...
  // Optional: Base color used by the boundary fade
  boundaryColor?: string;
  // Optional: The color of the lines (default is #c6c6c6)
  lineColor?: string;
  // Optional: The color of the big lines (default is #c6c6c6)
  bigLineColor?: string;
  // Optional: Initial progress value between 0 and 1
  initialProgress?: number;
  // Optional: Visible width of the slider viewport
...
export const DraggableSlider: React.FC<DraggableSliderProps> = ({
  ...
  const initialSpacePerLine =
    typeof spacePerLineProp === 'number' ? spacePerLineProp : spacePerLineProp.value;
  const clampedInitialProgress = clamp(initialProgress, 0, 1);
  const initialOffset =
    viewportWidth / 2 -
    Math.round(linesAmount * initialSpacePerLine) * clampedInitialProgress;

  const scrollContext = useSharedValue(0);
  const scrollOffset = useSharedValue(initialOffset);
  const clampedScrollOffset = useSharedValue(initialOffset);
  ...
  const spacings = useDerivedValue(() => {
    return linesArray.map(
      (_, i) => viewportWidth / 2 - i * spacePerLine.value * snapEach,
    );
  }, [linesArray, snapEach, spacePerLine, viewportWidth]);
  ...
  useAnimatedReaction(
    ...
    (offset, prev) => {
      ...
      const progress = interpolate(
        offset,
        [viewportWidth / 2, -progressWidth.value + viewportWidth / 2],
        [0, 1],
      );
  ...
  const gesture = Gesture.Pan()
    ...
    .onUpdate(event => {
      scrollOffset.value = clamp(
        ...
        -progressWidth.value + viewportWidth / 2,
        viewportWidth / 2,
      );
  ...
  }, [bigLineIndexOffset, linesAmount, maxLineHeight, minLineHeight, scrollOffset, spacePerLine, viewportWidth]);
              ...
              */}
              ...
              {showBoundaryGradient && (
                <BoundaryGradient
                  ...
                  height={maxLineHeight}
                  width={viewportWidth}
                  mainColor={boundaryColor ?? (!Color(lineColor).isLight() ? '#000000' : '#ffffff')}
                />
              )}
            </Canvas>
          </Animated.View>
        ...
        style={[
          {
            ...
            position: 'absolute',
            left: viewportWidth / 2 - indicatorLineWidth / 2,
            borderRadius: 10,
