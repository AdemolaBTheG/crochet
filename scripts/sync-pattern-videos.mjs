...
const DEFAULT_INPUT = resolve(process.cwd(), 'data/pattern-videos.json');
...
if (!SERVICE_ROLE_KEY) {
  console.error(
    'Missing SUPABASE_SERVICE_ROLE_KEY\n' +
      'Add it to .env before syncing pattern video IDs to Supabase.',
  );
  process.exit(1);
}
...
async function pushMap(filePath) {
  ...
  for (const [slug, value] of entries) {
    ...
    console.log(`OK ${slug} -> ${youtubeVideoId}`);
  }
}
...
async function pullMap(outputPath) {
  ...
  const payload = Object.fromEntries(
    (data ?? [])
      ...
      .map((row) => [row.slug, row.youtube_video_id]),
  );
...
if (command === 'push') {
  ...
  console.error('Usage: node scripts/sync-pattern-videos.mjs [push|pull] [path]');
  process.exit(1);
}
